...
public class Algorithm implements Brain {
            ...
            for (int j = -1; j <= 1; j++) {
                if (abs(abs(i) - abs(j)) == 1) {
                    ...
                    if (v != CellType.WALL && v != CellType.SNAKE) {
                        LinkedList<Point> path = findShortestPath(o, dst, world);
                        if (path != null) {
                            path.addFirst(src);
                            paths.add(path);
                        }
    ...
    public  LinkedList<Point> findSeriesPath(Point src, Point mid, Point dst, Snake snake) {
        ...
        var midToDstPath = findShortestPath(mid, dst, shadowSnake.getCellTypes());
